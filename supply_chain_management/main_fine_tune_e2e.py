...
"""
This script is used to fine-tune the offline trained E2E IQL model 
"""
...
if not args.test:
    ...
    model.load_checkpoint(
        path=f"/ckpt/{args.checkpoint_path}_running.pth"
    )
    ...
    for t in range(total_steps):
        ...
        action_output = model.select_action(state, deterministic=False)
        main_edges_act = {
        ...
        model.replay_buffer.store(
            state, torch.tensor(action_output).float(), args.rew_scale * reward, next_state, done
        )
        
        if t > 2 * args.batch_size:
            batch1 = Dataset.sample_batch(int(args.batch_size * counter), return_list=True)
        ...
        if done:
            if ep_ret >= best_reward:
                model.save_checkpoint(path=f"/ckpt/{args.checkpoint_path}_fine_sample.pth")
                best_reward = ep_ret
