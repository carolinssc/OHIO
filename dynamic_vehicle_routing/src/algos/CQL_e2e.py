...
"""
CQL E2E implementation for the vehicle routing problem
"""
...
class GNNActor(nn.Module):
    ...
    def forward(self, state, edge_index, deterministic=False):
        ...
        mu, sigma = F.softplus(self.h_to_mu(x)), F.softplus(self.h_to_sigma(x))

        if deterministic:
            action = mu
            log_prob = None
        else:
            m = Normal(loc=mu.squeeze(), scale=sigma.squeeze(-1))
            action = m.rsample()
            log_prob = m.log_prob(action)
            log_prob = log_prob.sum(dim=1, keepdim=True).squeeze()
...
class SAC(nn.Module):
    """
    SAC algorithm for the AMoD control problem.
    """
    ...
    def __init__(
        ...
        self.nvehicles = 0
        for n in env.G:
            self.nvehicles += env.G.nodes[n]["accInit"]
        self.nvehicles = self.nvehicles / 10
        # SAC parameters
        self.alpha = alpha
        self.polyak = polyak
        self.env = env
    ...
    def compute_loss_q(self, data, conservative=False, enable_calql=False):
        ...
        if conservative:
            ...
            (
                ...
                data, 10, batch_size, action_dim=len(self.env.edges)
            )

            if self.min_q_version == 2:
                cat_q1 = torch.cat(
                    [q1_rand, q1.unsqueeze(1).unsqueeze(1), q1_next, q1_current], 1
                )
                cat_q2 = torch.cat(
                    [q2_rand, q2.unsqueeze(1).unsqueeze(1), q2_next, q2_current], 1
                )
            ...
            reg = (
                self.critic1(
                    state_batch, edge_index, self.actor(state_batch, edge_index)[0]
                ).detach()
                - q1.detach()
            ).mean()
    ...
    def _get_action_and_values(self, data, num_actions, batch_size, action_dim):
        random_actions = (
            ...
            .uniform_(0, self.nvehicles)
            .to(self.device)
        )
        random_log_prob = math.log(0.5 ** (self.nvehicles))
