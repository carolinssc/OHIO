...
"""
CQL bi-level policy implementation for the vehicle routing problem
"""
...
class SAC(nn.Module):
    ...
    def update(self, data, conservative=False, enable_calql=False):
        loss_q1, loss_q2 = self.compute_loss_q(
            data, conservative, enable_calql)
    ...
    def _get_action_and_values(self, data, num_actions, batch_size, action_dim):
        # random_actions = (torch.FloatTensor(batch_size * self.num_random, action_dim).uniform_(-1, 1).to(self.device))
        alpha = torch.ones(action_dim)
        d = torch.distributions.Dirichlet(alpha)
        random_actions = d.sample((batch_size * self.num_random,))
        random_log_prob = (
            d.log_prob(random_actions).view(
