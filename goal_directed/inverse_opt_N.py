...
def N(model, K, s, s1, steps=1000):
    ...
    for epoch in range(steps): 
        ...
        for i in range(5):
            ...
            A, B = get_A_B_learned(model, x[i].detach().numpy())

            u_opt = torch.clamp(K[i] @ (x[i] - u_desired), -1, 1)
            # System dynamics
            x[i + 1] = A @ x[i] + B @ u_opt

        # Calculate MSE loss
        loss = nn.MSELoss()(x[5], s1)
...
if __name__ == "__main__":
    ...
    for j in range(states1.shape[0]):
        ...
        with open(f"data/data_preprocessed/reacher_hard_N_{j}.pkl", "wb") as f:
            pickle.dump(data, f)
