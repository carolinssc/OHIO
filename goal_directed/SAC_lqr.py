...
class SAC(nn.Module):
    ...
    def __init__(
        ...
        obs_dim = obs_dim
        act_dim = 4
        act_limit = act_limit.to(self.device)
        # Networks
        print(act_dim)
    ...
    def test_agent(self, env, lqr_steps=5):
        ...
        for _ in range(self.num_test_episodes):
            ...
            state = rl_agent.flatten_observation(time_step.observation)
            ...
            while not time_step.last():
                ...
                next_state = rl_agent.flatten_observation(time_step.observation)
...
if __name__ == "__main__":
    ...
    print(device)
    PATH = f"ckpt/SAC_{args.path}.pth"

    env = suite.load("reacher", "hard")
    
    obs_dim = 6
    act_dim = 4
    act_limit = torch.tensor([2 * np.pi, 2 * np.pi, 5, 5])
    ...
    if not args.test:
        ...
        for i in range(total_steps):
            ...
            while not time_step.last() and not terminated:
                ...
                ep_ret += rew
                rl_agent.memory.push(state, rl_action, next_state, rew, False)
            ...
            if i % 10 == 0 and i != 0:
                test_rew = rl_agent.test_agent(env=env, lqr_steps=lqr_steps)
