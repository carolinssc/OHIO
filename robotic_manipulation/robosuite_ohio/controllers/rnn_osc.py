...
class RNN_osc_controller(Controller):
    ...
    def __init__(
        ...
        **kwargs,  # does nothing; used so no error raised when dict is passed with extra terms used previously
        ...
        self.model = ObservationConditionedRNN(input_dim=34, hidden_dim=256, output_dim=7)
        checkpoint = torch.load("/mnt/raid1/csasc_storage/RNN_door_osc_final.pth")
    ...
    def run_controller(self):
        ...
        #np.concatenate([data['observations'], data['quats']], axis=-1)

        osc_obs = np.concatenate([self.ee_pos, T.mat2quat(self.ee_ori_mat)])
        rnn_input = np.concatenate([qpos,self.joint_vel[:7],osc_obs, self.goal_pos], axis=-1) 
