...
class Controller(object, metaclass=abc.ABCMeta):
    ...
    def update_state_from_dataset(self, state):
        """
        Updates the simulation state based on the given dataset state.

        Args:
            state (dict): State from the dataset with keys 'qpos' and 'qvel'.
        """
        # Update only the relevant indices of qpos and qvel
        #self.sim.data.qpos[self.qpos_index] = state['qpos'][self.qpos_index]
        #self.sim.data.qvel[self.qvel_index] = state['qvel'][self.qvel_index]

        self.sim.data.qpos[:] = state['qpos']
        self.sim.data.qvel[:] = state['qvel']
        
        # Update gripper states
        #self.gripper_indices = [7,8]
        #self.sim.data.qpos[self.gripper_indices] = state['qpos'][self.gripper_indices]
        #self.sim.data.qvel[self.gripper_indices] = state['qvel'][self.gripper_indices]
        #idx = len(state['qpos'])
        #self.sim.data.qpos[:9] = state['qpos']
        #self.sim.data.qvel[:9] = state['qvel']
    ...
    def update(self, force=False):
        ...
        if self.new_update or force:
            ...
            self.ee_pos_vel = np.array(self.sim.data.get_site_xvelp(self.eef_name)) #linear velocity
            self.ee_ori_vel = np.array(self.sim.data.get_site_xvelr(self.eef_name)) #angular velocity
            ...
            mass_matrix = numpy.ndarray(shape=(self.sim.model.nv, self.sim.model.nv), dtype=np.float64, order="C")
