...
from robosuite_ohio.utils.buffers import RingBuffer
import torch 
...
class JointVelocityController(Controller):
    ...
    def set_goal_diff(self, velocities):
        """
        Sets goal based on input @velocities.
        ...
        action = torch.clip(velocities, torch.tensor(self.input_min), torch.tensor(self.input_max))
        self.goal_vel = (action - torch.tensor(self.action_input_transform)) * torch.tensor(self.action_scale) + torch.tensor(self.action_output_transform)

        if self.velocity_limits is not None:
            self.goal_vel = torch.clip(self.goal_vel, torch.tensor(self.velocity_limits[0]), torch.tensor(self.velocity_limits[1]))

        if self.interpolator is not None:
            print('set goal interpolator')
            self.interpolator.set_goal_diff(self.goal_vel)
    ...
    def run_controller_diff(self):
        self.summed_err = torch.tensor(self.summed_err)
        """
        Calculates the torques required to reach the desired setpoint
        ...
        if self.interpolator is not None:
            if self.interpolator.order == 1:
                # Linear case#
                print('interpolator')
                self.current_vel = self.interpolator.get_interpolated_goal_diff()
                print(self.current_vel)
            else:
                # Nonlinear case not currently supported
                pass
        else:
            self.current_vel = self.goal_vel

        # Compute necessary error terms for PID velocity controller
        err = self.current_vel - torch.tensor(self.joint_vel)
        derr = err - torch.tensor(self.last_err)
        self.last_err = err
        #print(self.last_err , 'last_err')
        self.derr_buf.push(derr.detach().numpy())
        ...
        # Compute command torques via PID velocity controller plus gravity compensation torques
        torques = torch.tensor(self.kp) * err + torch.tensor(self.ki) * torch.tensor(self.summed_err) + torch.tensor(self.kd) * torch.tensor(self.derr_buf.average) + torch.tensor(self.torque_compensation)

        # Clip torques
        self.torques = torch.clip(torques, torch.tensor(self.actuator_min), torch.tensor(self.actuator_max))

        # Check if we're saturated
        self.saturated = False if torch.sum(torch.abs(self.torques - torques)) == 0 else True
        #self.saturated = False if np.sum(np.abs(self.torques - torques)) == 0 else True
