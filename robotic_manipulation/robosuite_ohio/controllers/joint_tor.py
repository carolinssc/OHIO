...
class JointTorqueController(Controller):
    ...
    def set_goal_diff(self, torques):
        """
        Sets goal based on input @torques.
        ...
        # Check to make sure torques is size self.joint_dim
        assert len(torques) == self.control_dim, "Delta torque must be equal to the robot's joint dimension space!"
        #self.action_scale = [[80., 80., 80., 80., 80., 12., 12.], [80., 80., 80., 80., 80., 12., 12.]]
        ...
        action = torch.clip(torques, torch.tensor(self.input_min), torch.tensor(self.input_max))
        #print('clipped action', action)
        torques = (action - torch.tensor(self.action_input_transform)) * torch.tensor(self.action_scale) + torch.tensor(self.action_output_transform)
        #print('scaled action', torques)
        self.goal_torque = torch.clip(torques, torch.tensor(self.torque_limits[0]), torch.tensor(self.torque_limits[1]))
        #print('clipped goal_torque', self.goal_torque)
        #print('interpolator', self.interpolator)
        #print(self.goal_torque)
        if self.interpolator is not None:
            self.interpolator.set_goal_diff(self.goal_torque)
    ...
    def run_controller_diff(self):
        ...
        if self.interpolator is not None:
            # Linear case
            if self.interpolator.order == 1:
                self.current_torque = self.interpolator.get_interpolated_goal_diff()
            else:
                # Nonlinear case not currently supported
                pass
        else:
            self.current_torque = self.goal_torque
            #print('current_torque', self.current_torque)
        
        # Add gravity compensation
        #print('current_torque', self.current_torque)
        #print('torque_compensation', self.torque_compensation)
        self.torques = self.current_torque + torch.tensor(self.torque_compensation)
