...
def quat2mat_diff(quaternion):
    ...
    """
    # awkward semantics for use with numba
    inds = torch.tensor([3, 0, 1, 2], dtype=torch.long)
    q = quaternion[inds]
    ...
    q2 = torch.outer(q, q)  # Outer product of q with itself
        
    rotation_mat_error = torch.empty((3, 3), dtype=q.dtype, device=q.device)
    rotation_mat_error[0, 0] = 1.0 - q2[2, 2] - q2[3, 3]
    rotation_mat_error[0, 1] = q2[1, 2] - q2[3, 0]
    rotation_mat_error[0, 2] = q2[1, 3] + q2[2, 0]
    rotation_mat_error[1, 0] = q2[1, 2] + q2[3, 0]
    rotation_mat_error[1, 1] = 1.0 - q2[1, 1] - q2[3, 3]
    rotation_mat_error[1, 2] = q2[2, 3] - q2[1, 0]
    rotation_mat_error[2, 0] = q2[1, 3] - q2[2, 0]
    rotation_mat_error[2, 1] = q2[2, 3] + q2[1, 0]
    rotation_mat_error[2, 2] = 1.0 - q2[1, 1] - q2[2, 2]

    return rotation_mat_error
