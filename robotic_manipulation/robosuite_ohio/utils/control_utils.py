...
import robosuite_ohio.utils.transform_utils as trans
from robosuite_ohio.utils.numba import jit_decorator
import torch
...
def nullspace_torques_diff(mass_matrix, nullspace_matrix, initial_joint, joint_pos, joint_vel, joint_kp=10):

    # kv calculated below corresponds to critical damping
    joint_kv = torch.sqrt(torch.tensor(joint_kp)) * 2
...
def opspace_matrices_diff(mass_matrix, J_full, J_pos, J_ori):
    mass_matrix_inv = torch.inverse(mass_matrix)

    # J M^-1 J^T
    lambda_full_inv = J_full @ mass_matrix_inv @ J_full.transpose(-1, -2)

    # Jx M^-1 Jx^T
    lambda_pos_inv = J_pos @ mass_matrix_inv @ J_pos.transpose(-1, -2)

    # Jr M^-1 Jr^T
    lambda_ori_inv = J_ori @ mass_matrix_inv @ J_ori.transpose(-1, -2)
    ...
    # nullspace
    Jbar = mass_matrix_inv @ J_full.transpose(-1, -2) @ lambda_full
    nullspace_matrix = torch.eye(J_full.shape[-1]) - Jbar @ J_full
...
def set_goal_position_diff(delta, current_position, position_limit=None, set_pos=None):
    """
    Calculates and returns the desired goal position, clipping the result accordingly to @position_limits.
    @delta and @current_position must be specified if a relative goal is requested, else @set_pos must be
    specified to define a global goal position
    ...
    if position_limit is not None:
        ...
        # Clip goal position
        goal_position = torch.clip(goal_position, position_limit[0], position_limit[1])
...
def set_goal_orientation(delta, current_orientation, orientation_limit=None, set_ori=None):
    ...
    #print('delta', delta)
    #print('orientation_limit', orientation_limit)
    # directly set orientation
    if set_ori is not None:
        goal_orientation = set_ori
...
def set_goal_orientation_diff(delta, current_orientation, orientation_limit=None, set_ori=None):
    """
    Calculates and returns the desired goal orientation, clipping the result accordingly to @orientation_limits.
    @delta and @current_orientation must be specified if a relative goal is requested, else @set_ori must be
    an orientation matrix specified to define a global orientation
    ...
    # otherwise use delta to set goal orientation
        ...
        quat_error = trans.axisangle2quat_diff(delta)
        rotation_mat_error = trans.quat2mat_diff(quat_error)

        goal_orientation = torch.matmul(rotation_mat_error, torch.tensor(current_orientation, dtype=torch.float64))
