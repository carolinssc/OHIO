...
import robosuite_ohio
import robosuite_ohio.macros as macros
import robosuite_ohio.utils.sim_utils as SU
from robosuite_ohio.renderers.base import load_renderer_config
from robosuite_ohio.utils import OpenCVRenderer, SimulationError, XMLError
from robosuite_ohio.utils.binding_utils import MjRenderContextOffscreen, MjSim
...
class MujocoEnv(metaclass=EnvMeta):
    ...
    def initialize_renderer(self):
        ...
        if self.renderer == "mujoco" or self.renderer == "default":
            ...
            from robosuite_ohio.renderers.nvisii.nvisii_renderer import NVISIIRenderer
    ...
    def step(self, action, verbose=False):
        ...
        if verbose: 
            torques_all_steps = []
            observables_all_steps = []
            observables_all_steps.append(self._get_observations(force_update=True)['robot0_eef_pos'])
            q_pos_all_steps = []
            q_pos_all_steps.append(np.copy(self.sim.data.qpos))
            q_veq_all_steps = []
            q_veq_all_steps.append(np.copy(self.sim.data.qvel))
            quaternion_all_steps = []
            quaternion_all_steps.append(np.copy(self._get_observations(force_update=True)['robot0_eef_quat']))
        for i in range(int(self.control_timestep / self.model_timestep)):
            self.sim.forward()
            self._pre_action(action, policy_step)
            if verbose: 
                torques_all_steps.append(np.copy(self.sim.data.ctrl))
            self.sim.step()
        ...
        if verbose: 
            info['torques'] = torques_all_steps
            info['observations'] = observables_all_steps
            info['q_pos'] = q_pos_all_steps
            info['q_vel'] = q_veq_all_steps
            info['quaternions'] = quaternion_all_steps
        if self.viewer is not None and self.renderer != "mujoco":
            self.viewer.update()
    ...
    def edit_model_xml(self, xml_str):
        ...
        path = os.path.split(robosuite_ohio.__file__)[0]
