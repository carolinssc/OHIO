...
from robosuite_ohio.robots.robot import Robot

try:
    import igibson
except ImportError:
    raise Exception(
        "Use `pip install igibson` to install iGibson renderer. "
...
from robosuite_ohio.renderers import load_renderer_config
from robosuite_ohio.renderers.base import Renderer
from robosuite_ohio.renderers.igibson.igibson_utils import TensorObservable, adjust_convention, MujocoRobot
from robosuite_ohio.renderers.igibson.parser import Parser
from robosuite_ohio.utils import macros
from robosuite_ohio.utils import transform_utils as T
from robosuite_ohio.utils.mjcf_utils import IMAGE_CONVENTION_MAPPING
from robosuite_ohio.utils.observables import sensor
...
def check_camera_obs(use_camera_obs):
    if use_camera_obs:
        raise ValueError(
            "Cannot set `use_camera_obs` in the environment, instead set 'camera_obs' flag in iGibson initialization."
        )
...
def check_segmentation_type(segmentation_type):
    ...
    if segmentation_type[0] is not None:
        assert len(segmentation_type[0]) == 1, ValueError(
            "Only one sensor per camera can be used."
...
class iGibsonRenderer(Renderer):
    def __init__(
        ...
        self.renderer = self.renderer_class(
            width=width, height=height, vertical_fov=45, device_idx=device_idx, rendering_settings=self.mrs
        )
        ...
        # add viewer
        self.viewer = self._add_viewer(initial_pos=self.camera_position, initial_view_direction=self.view_direction)
    ...
    def _setup_observables(self):
        ...
        if self.camera_obs:
            ...
            for (cam_name, cam_d) in zip(self.env.camera_names, self.env.camera_depths):
    ...
    def _create_camera_sensors(self, cam_name, cam_w, cam_h, cam_d, modality="image"):
        ...
        if "seg" in self.modes:
            ...
            def camera_seg(obs_cache):
                return obs_cache[seg_sensor_name] if seg_sensor_name in obs_cache else np.zeros((cam_h, cam_w, 1))
        ...
        if "normal" in self.modes:
            ...
            def camera_normal(obs_cache):
                return obs_cache[normal_sensor_name] if normal_sensor_name in obs_cache else np.zeros((cam_h, cam_w, 1))
    ...
    def _add_viewer(self, initial_pos=[0, 0, 1], initial_view_direction=[1, 0, 0], initial_up=[0, 0, 1]):
        """
